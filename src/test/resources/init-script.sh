...
# Creating Index
    curl -XPUT "${ELASTIC_HOST}/${IDXNAME}?pretty" -H 'Content-Type: application/json' -d " { \"settings\" : { \"index\" : { \"number_of_shards\" : 3, \"number_of_replicas\" : 1 } } }"
}
...
function createManager() {
    ...
    curl -XPUT "${ELASTIC_HOST}/${INDEX}/${MANAGERTYPEMAPPING}/${ID}?pretty" -H 'Content-Type: application/json' -d" { \"name\" : \"${NAME}\", \"employeeId\" : ${EMPLOYEEID}, \"department\" : \"${DEPARTMENT}\" }"
}
...
function createDeveloper() {
    ...
    DEPARTMENT=$4
    REPORTSTO=$5
    curl -XPUT "${ELASTIC_HOST}/${INDEX}/${DEVELOPERTYPEMMAPPING}/${ID}?pretty" -H 'Content-Type: application/json' -d" { \"name\" : \"${NAME}\", \"employeeId\" : ${EMPLOYEEID}, \"department\" : \"${DEPARTMENT}\", \"reportsTo\" : \"${REPORTSTO}\" }"
}

#Clean Index
curl -XDELETE "${ELASTIC_HOST}/${INDEX}?pretty"
