...
public class ElasticSearchGroupScan extends AbstractGroupScan {
	...
	public ElasticSearchGroupScan(String userName, ElasticSearchStoragePlugin plugin, ElasticSearchScanSpec scanSpec,
	...
	private static final Comparator<List<ElasticSearchScanSpec>> LIST_SIZE_COMPARATOR_REV = Collections
	...
	public void applyAssignments(List<DrillbitEndpoint> incomingEndpoints) {
		...
		Preconditions.checkArgument(numSlots <= regionsToScan.size(), String.format(
				"Incoming endpoints %d is greater than number of scan regions %d", numSlots, regionsToScan.size()));
		 ...
		 */
		for (Iterator<Entry<PartitionDefinition, ServerHost>> regionsIterator = regionsToAssignSet.iterator(); regionsIterator
				.hasNext(); /* nothing */) {
		 ...
		 */
		PriorityQueue<List<ElasticSearchScanSpec>> minHeap = new PriorityQueue<List<ElasticSearchScanSpec>>(numSlots,
				LIST_SIZE_COMPARATOR);
		PriorityQueue<List<ElasticSearchScanSpec>> maxHeap = new PriorityQueue<List<ElasticSearchScanSpec>>(numSlots,
	...
	private ElasticSearchScanSpec regionInfoToSubScanSpec(PartitionDefinition part) {
		...
		ElasticSearchScanSpec spec = new ElasticSearchScanSpec(scanSpec.getIndexName(), scanSpec.getTypeMappingName(),
				part);
	...
	public ScanStats getScanStats() {
		...
		try {
			...
			if (!countNode.isMissingNode()) {
				...
				logger.warn("There are no documents in {}.{}?", this.scanSpec.getIndexName(),
						this.scanSpec.getTypeMappingName());
			}
			long docSize = 0;
			if (numDocs > 0) {
				response = client.performRequest("GET",
				...
				if (!hits.isMissingNode()) {
					...
					throw new DrillRuntimeException("Couldn't size any documents for " + this.scanSpec.getIndexName()
							+ "." + this.scanSpec.getTypeMappingName());
				}
			}
			// 这样就知道有多少数据量了
